...
- Datasets, die auf einer Liveverbindung mit einem Power BI-Dataset in einem anderen Arbeitsbereich basieren. Weitere Informationen finden Sie unter [Einführung in die Verwendung von Datasets in mehreren Arbeitsbereichen](service-datasets-across-workspaces.md).
...
Andere Benutzer, die über eine [Berechtigung „Erstellen“](service-datasets-build-permissions.md) für ein Dataset verfügen, sind mit Analysis Services-Datenbanklesern gleichgestellt. Sie können eine Verbindung mit Datasets herstellen und Daten für die Nutzung und Visualisierung durchsuchen. Regeln für die Sicherheit auf Zeilenebene (Row-Level Security, RLS) werden berücksichtigt, und es werden keine internen Datasetmetadaten angezeigt.
...
XMLA-Schreibvorgänge für Datasets, die in Power BI Desktop erstellt und in einem Premium-Arbeitsbereich veröffentlicht wurden und erweiterte Metadaten erfordern, sind aktiviert. Weitere Informationen finden Sie unter [Aktivieren erweiterter Datasetmetadaten](desktop-enhanced-dataset-metadata.md).
...
Power BI Desktop kann eine Verbindung zu einem Power BI Premium-Dataset herstellen, als ob es sich um eine Modelldatenbank handeln würde, die bei Azure Analysis Services oder SQL Server Analysis Services bereitgestellt wird. In diesem Fall verwendet Power BI Desktop den XMLA-Endpunkt. Es wird jedoch empfohlen, dass Power BI Desktop-Benutzer stattdessen die Liveverbindungsfunktion verwenden, die speziell für Power BI-Datasets erstellt wurde. Durch die Liveverbindung wird das Auffinden von Datasets verbessert, und die Benutzer müssen die URLs der Arbeitsbereiche nicht mehr im Auge behalten; sie können einfach den Namen des Datasets eingeben. Weitere Informationen finden Sie unter [Herstellen einer Verbindung mit Datasets im Power BI-Dienst über Power BI Desktop](desktop-report-lifecycle-datasets.md).
