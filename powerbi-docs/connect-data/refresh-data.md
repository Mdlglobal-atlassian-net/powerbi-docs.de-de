...
Ein Power BI-Dataset kann in einem der folgenden Modi für den Zugriff auf Daten aus einer Vielzahl von Datenquellen eingesetzt werden. Weitere Informationen erhalten Sie unter [Storage mode in Power BI Desktop (Speichermodus in Power BI Desktop)](../transform-model/desktop-storage-mode.md).
...
## <a name="refresh-and-dynamic-data-sources"></a>Aktualisieren dynamischer Datenquellen
...
![Indikator für dynamische Datenquellen](media/refresh-data/dynamic-data-source.png)
...
Weitere Informationen zur automatischen Seitenaktualisierung finden Sie im Artikel [Automatische Seitenaktualisierung](../create-reports/desktop-automatic-page-refresh.md).
...
- Begrenzen Sie die Anzahl der visuellen Elemente auf Ihren Dashboards, insbesondere wenn Sie [Sicherheit auf Zeilenebene (Row-Level Security; RLS) mit Power BI](../admin/service-admin-rls.md) verwenden. Wie bereits zuvor in diesem Artikel erläutert, kann eine übermäßige Anzahl von Dashboardkacheln die Aktualisierungsdauer erheblich erhöhen.
